...
bool Planet::operator<(const Planet& p) const {
	return PlanetID() < p.PlanetID();
}
...
void PlanetWars::Planets(const std::vector<Planet>& peas)
{
	planets_.clear();
  for (int i = 0; i < peas.size(); ++i) {
    const Planet& p = peas[i];
...
void PlanetWars::AddFleets(std::vector<Fleet> fleas)
{
  for (int i = 0; i < fleas.size(); ++i) {
    const Fleet& f = fleas[i];
    fleets_.push_back(f);
  }
}
