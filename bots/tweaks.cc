...
void DoTurn(const PlanetWars& pw) {
	...
	//testing to see if I should wait a bit longer before doing this.
	if(totalTurnsPassed > 15)
		rMoves = getReinforcementMoves(currentpw, futurepw);
...
int main(int argc, char *argv[]) {
	debug.close();
	debug.open("bots/debug/tweaks.log");
	debugdetails.close();
	debugdetails.open("bots/debug_details/tweaks.log");
